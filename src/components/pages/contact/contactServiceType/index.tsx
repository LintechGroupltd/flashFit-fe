...
const ContactServiceType = ({handleStepChange,handleOptionSelect}:StepProps) => {
  return (
    <div className="font-serif w-full  xl:w-3/5 flex flex-col gap-20 items-center" data-aos='fade-up'>
      ...
      <div>
        <ul className="flex items-center justify-center flex-wrap gap-4 sm:gap-10">
          {contactServiceOptions.map((item, idx) => (
            <li key={idx} className="text-xs sm:text-base cursor-pointer px-4 py-4 md:px-10 md:py-6 rounded-lg bg-white bg-opacity-10 border border-white hover:bg-opacity-5"
             onClick={() =>{
              handleStepChange(1);
